...
import ProductLottery from '@/components/lottery';
...
export default async function ProductPage({params}:Props){const p=await getProduct((await params).id);const base=process.env.SITE_URL||'http://localhost:3000';const schema={'@context':'https://schema.org','@type':'Product',name:p.name,image:new URL(p.image,base).href,description:p.tags.join('、'),offers:{'@type':'Offer',url:`${base}/products/${p.id}`,priceCurrency:'CNY',price:(p.price/100).toFixed(2),availability:p.stock>0?'https://schema.org/InStock':'https://schema.org/OutOfStock'}};return <main id="main" className="site-main"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,'\\u003c')}}/><Link className="back-link" href="/"><ArrowLeft size={17}/>返回商品中心</Link>{p.product_type==='lottery'?<ProductLottery product={p}/>:<Checkout product={p}/>}</main>;}
