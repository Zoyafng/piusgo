    ...
    def enter(pid:int,body:DrawInput,u=Depends(member)):
        ...
        with db(True) as c:
            ...
            if won:
                ...
                c.execute("INSERT INTO orders(id,user_id,product_id,quantity,account,total,discount,status,created,request_key,variant_id,variant_name,payment_method,source,product_name,product_image) VALUES(:id,:uid,:pid,1,:email,0,:discount,'pending',:now,:key,:vid,:name,'mock','lottery',:product_name,:product_image)",{'id':oid,'uid':u['id'],'pid':pid,'email':u['email'],'discount':v['price'],'now':now,'key':'lottery:'+secrets.token_hex(16),'vid':v['id'],'name':v['name'],'product_name':p['name'],'product_image':p['image']})
